...
The work going on with WASM and [wasm-bindgen](https://github.com/rustwasm/wasm-bindgen) makes this layer particularly interesting when considering true cross platform reuse of core logic. Imagine that you've built a BARE application in C++, Java or .Net and now need to port it to the web. Wouldn't it be nice to have a nice set of isolated core components that could be ported directly and all maintained in the same code repository?  
...
Certain languages look like they're going to be around a while. C++, Java, JavaScript, C# - they're all going to be around for at least the next ten years, and that's all we can really hope for as far as longevity for an application (though, horrifically, I know of old VB code that I wrote almost 20 years ago still running in production!). If we use simple constructs, stick with the vanilla language and don't escape core principles we can develop software that will be flexible and maintainable far into the future.
